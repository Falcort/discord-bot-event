...
export class EventsServiceClass {
  ...
  public async getEvents(): Promise<EventInterface[]> {
    ...
    try {
      ...
      const request = await Axios.get(
        `${this.GLOBALS.API_URL}/content-manager/collection-types/application::dbe-events.dbe-events?event_date_gte=${date}`,
        { headers: { Authorization: `Bearer ${this.GLOBALS.JWT}` } },
      );
      result = request.data.results;
    } catch (e) {
      Logger.error(
        `Exception in getEvents() :\n ${
          e.response ? JSON.stringify(e.response.data) : e
        }`,
      );
    }
  ...
  public async putEventParticipants(
    ...
    try {
      const request = await Axios.put(
        `${this.GLOBALS.API_URL}/content-manager/collection-types/application::dbe-events.dbe-events/${eventID}`,
        { participants },
        { headers: { Authorization: `Bearer ${this.GLOBALS.JWT}` } },
      );
      result = request.data;
      ...
      Logger.error(
        `Exception in putEventParticipants() :\n ${
          e.response ? JSON.stringify(e.response.data) : e
        }`,
      );
    }
   ...
   */
  public async getEventFromMessageID(
    messageID: string,
  ): Promise<EventInterface> {
    let result = null;
    try {
      const request = await Axios.get(
        `${this.GLOBALS.API_URL}/content-manager/collection-types/application::dbe-events.dbe-events?message_id_eq=${messageID}`,
        { headers: { Authorization: `Bearer ${this.GLOBALS.JWT}` } },
      );
      result = request.data.results.pop() || [];
    } catch (e) {
      Logger.error(
        `Exception in getEventFromMessageID() :\n ${
          e.response ? JSON.stringify(e.response.data) : e
        }`,
      );
    }

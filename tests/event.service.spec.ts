...
describe('[Service] Events', () => {
  describe('postEvent()', () => {
    it('valid', async () => {
      ...
      const result = await EventsService.postEvent(
        '',
  ...
  describe('getEvents()', () => {
    ...
    it('two', async () => {
      ...
      mockedAxios.get.mockResolvedValue({
        data: { results: [constantMocks.event, constantMocks.event] },
      });
      const result = await EventsService.getEvents();
  ...
  describe('putEventParticipants()', () => {
    it('valid', async () => {
      ...
      // eslint-disable-next-line max-len
      const result = await EventsService.putEventParticipants(
        { users: constantMocks.eventInterface.participants },
        constantMocks.eventInterface.id,
      );
      expect(result).not.toBeNull();
    });
    it('error', async () => {
  ...
  describe('getEventFromMessageID()', () => {
    it('empty', async () => {
      expect.assertions(1);
      mockedAxios.get.mockResolvedValue({ data: { results: [] } });
      const result = await EventsService.getEventFromMessageID(
        constantMocks.message.id,
      );
      expect(result).toStrictEqual([]);
    });
    it('one', async () => {
